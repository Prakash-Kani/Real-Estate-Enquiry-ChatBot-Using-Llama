...
def search_with_query(search, user_query):
    if user_query.strip():
        return search.run( query=user_query)
    else:
        return "No properties match the specified criteria."

# Function to create the chain with Google Search retriever
def run_create_chain(api_key, cse_id):
    ...
    chain = (
        {
            "context": RunnableLambda(lambda x: search_with_query(search = retriever, user_query = x)),  # The retriever fetching the context (property listings)
